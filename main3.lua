...
room {
	...
	n_to = 'base';
	in_to = 'drag_door';
	dsc = [[Ты находишься на посадочной площадке базы. Здесь стоит китайский лунный модуль "Дракон". Можно вернуться на север к базе.]];
}: with {
	'moonsky',
	'пыль',
	'грунт',
...
-- 2) после возвращения
-- 3) в драконе - Лю Ливей в скафандре, приносят, раздевают, кладут на кровать - он в коматозе.
-- 4) осматривают радиотелескоп на обрыве
-- 3) возвращаются на базу, лю ливея нет. Снимают скафандры (зачем?) дышат газом и засыпают.
-- 4) разобрать аккумуляторы с солнечной станции, панель управления модулем.