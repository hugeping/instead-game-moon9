	...
	obj {
		...
		obj { nam = 'пеленгатор',
			...
			description = function(s)
				...
				if s:has 'on' then
					if here()^'base' then
						p [[Лунный модуль "Арго" находится на востоке.]]
...
Distance { nam = 'moonsky', -"небо|звёзды/но,мн", description = [[Яркий солнечный свет, отражённый от поверхности Луны, мешает тебе наблюдать звёзды.]]; }:with {
	Distance { -"Земля|серп", description = [[Голубой серп Земли едва заметен в чёрном небе.]] };
...
room {
	...
	Furniture {
		-"кресло";
		description = [[На китайском лунном модуле больше места, здесь используются нормальные кресла вместо стоек. Но кресла
		второго пилота нет! Похоже, что кто-то демонтировал его. Так же, как и оборудование.]];
	}:attr'scenery';
	Careful {
		-"окна|окно";
		description = [[Здесь окна больше, чем на модуле "Арго". Некоторое время ты рассматриваешь унылый лунный пейзаж.]];
	};
	obj {
		-"панель,отверст*,компьют*,пульт*|приборы";
		...
		if w ^ '@w_to' or w ^ 'пар' then
			...
			if not visited 'moonw' then
				if _'пеленгатор':hasnt'on' then
					p [[Без пеленгатора ориентироваться внутри этого странного облака будет сложно.]]
				else
					walkin 'moonw'
				end
				return
			else
				p [[Ты едешь нв луноходе к базе.]]
				move('buggy', 'base')
			end
			return
		end
		...
		description = function(s)
			...
			elseif s:inroom() ^ 'moon1' and not _'buggy'.assembled then
				...
				if s.suit then
					...
					if mission then
						p [[Глядя на спящего Александра, тебя жжёт совесть, но лунная принцесса обещала тебе избавить тебя от мук. После того, как ты выполнишь свою миссию.]]
						return
					end
...
cutscene {
	...
	title = 'Луна-9';
	text = {
		[[Пик Малаперта или пик вечного света -- вовышение у края кратера Малаперта. С этой точки Земля находится всегда
...
cutscene {
	...
	ощуимо подбрасывало на неровностях ландшафта. С некоторой тревогой вы приближались к странным розовым клубам пара, озаряемым яркими вспышками света.]];
...
room {
	...
	obj {
		...
		obj {
			-"скафандры";
			nam = 'suits';
			description = [[Эти скафандры ничем не отличаются от твоего.]];
		};
	}
}
door {
...
room {
	...
	["before_Walk,Enter"] = function(s, w)
		...
		if w ^ '@e_to' or w ^ '@w_to' then
			...
			if w ^ '@e_to' then
				p [[Ты едешь на луноходе к лунному модулю "Арго".]]
				move('buggy', 'moon1')
			elseif w ^ '@w_to' then
	...
	Careful {
		...
		description = function(s)
			if s.broken then
				p [[Радимаяк вскрыт и из него извлечён контроллер заряда, так что больше он не заряжается. Интересно, на сколько хватит заряда?]]
				return
			end
		...
		after_Receive = function(s, w)
			if w ^ 'контроллер' then
				p [[Ты вставил контролеер в радиомаяк.]]
				return
			end
	...
	Useless {
		-"модули";
		description = [[Модули врыты в грунт и покрыты антиметеоритной защитой.]];
	};
	Useless {
		-"защита,анти*";
...
room {
	...
	Careful {
		nam = 'powergen';
		-"электростанция,станция|управляющий модуль,модуль";
		before_SwitchOn = function(s)
			if s:has'on' then
				return false
			end
		...
		before_LetGo = function(s, w)
			if w ^'контроллер' and s:has'on' then
				p [[Сначала надо выключить станицю.]]
				return
			end
			return false
		end;
...
room {
	...
	obj {
		...
		description = function(s)
			...
			if mission then
				p [[^^С тоской в сердце ты понимаешь, что на постере изображена лунная принцесса. Но какая разница? Лунная принцесса сказала, что в её мире возможно всё.]]
			end
		end;
	...
	Furniture {
		...
		description = [[Небольшое и не слишком удобное.]];
	}:attr'supporter,enterable';
	Furniture {
		-"диванчик,диван";
		inside_dsc = [[Ты развалился на диванчике.]];
		description = [[Ты не ожидал увидеть такой диванчик на лунной станции.]];
	}:attr'supporter,enterable';
	Furniture {
		-"стол";
		inside_dsc = [[Ты стоишь на столе.]];
		description = [[Стол сделан из пластика.]];
...
cutscene {
	...
	text = {
		[[Ты выбрал одну из кроватей и сразу повалился на неё, даже не сняв ботки.^
...
room {
	...
	dsc = function(s)
		p [[Ты стоишь посреди огромного, искрящегося серебром зала. К высокому своду зала взлетают
	...
	Careful { -"дорожка|ковёр", description = [[Ты видишь, что над дорожкой клубится розоватый пар.]]; before_Enter = [[Ты и так стоишь на ковровой дорожке.]] };
		...
		obj {
			...
			daemon = function(s)
				...
				if s:inside'трон' and s.step == 1 then
					...
					elseif s.step == 3 then
						...
						-- Я вижу, что ты готов, я прочитала ответ в твоём сердце. Ты -- мой. Когда вернёшься в свой мир, ты будешь знать всё, что нужно сделать. И ты сделаешь всё как надо! И тогда, вернувшись ко мне, ты получишь всё то, о чём так долго мечтал, но не смеял надеяться.]]
						s.step = 4
					elseif s.step == 4 then
						p [[Девушка обнимает тебя левой рукой. По твоему лицу стекают капельки пота.^
						-- Все раны будут залечены. Всё, что тебя терзало в прошлом -- забыто. В мире, который твоя раса называет сном нет границ.  Но он так же реален, как и ваш. К счастью, твоя раса не знает об этом. Пока.]]
						s.step = 5
			...
			description = function(s)
				...
				if v.step == 1 then
					...
					p [[Станно, но тебе кажется, что ты её где-то видел раньше. Только тебе сложно вспомнить, где именно.]]
					DaemonStart(s)
				end
			end;
...
-- На пике в тени -- радиотелескоп.
-- 0) путь к посадочной площадке есть сразу и там просто разобранный "Дракон"
-- 1) после осмотра базы можно поехать на пик к радиотелескопу
-- 2) база разобрана, в том числе и станция на солнечных батареях, чтобы попасть внутрь нужна запасная микросхема, которая, возможно, находится в Драконе.
-- 3) на пике -- следы лунохода, который тоже разобран на теневой стороне. Сняты все аккумуляторы
-- 4) в кресле -- Лю Ливей, в скафандре, в коматозе
-- 5) привозят Лю Ливея в жилой блок
-- 6) ложатся спать