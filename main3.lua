...
room {
	...
	dsc = function(s)
		...
		if mission then
			p [[Твоё внимание привлекает панель управления.]]
		end
		_'#win':dsc()
	...
	Careful {
		...
		before_Attack = function(s, w)
			...
			_'запчасти'.got = true
			take 'запчасти'
			p [[Содрагаясь от того, что ты делаешь, ты разбираешь панель и демонтируешь из неё нужные запчасти. Ты не знаешь точно, что именно ты делаешь. Но {$fmt em|они} подсказыват тебе.]]
		end;
		prog = 1;
		daemon = function(s)
			local m = _'moonmod'
