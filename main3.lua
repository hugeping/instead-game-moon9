...
room {
	...
	Careful {
		-"сооружение",
		description = [[Тебе кажется, что всё происходящее -- сон.]];
	};
}
room {
	...
	Careful {
		-"батареи";
		description = [[Здесь всегда светло. Солнечных батарей достаточно, чтобы записать маяк.]];
	};
}
...
room {
	...
	dsc = function(s)
		...
		Можно спуститься вниз в технический отсек.]]
	end;
}:with {
		...
		daemon = function(s)
			...
			if gravity and _'moonmod'.height == 0 then
				...
				elseif here() == s:inroom() and not me():inside'buggy' and s:inside'buggy' then
					...
					move(s, here())
				elseif player_moved() and _'alex'.state == 7 and here():hasnt'cutscene' and not s:inside'buggy' then
...
room {
	...
	Careful {
		-"маяк,фонарь";
		description = [[Это радиомаяк, который и привёл вас к базе. Также он снабжён оптическим светодиодным пульсирующим фонарём.]];
	};
	Useless {
		-"грунт";
		nam = 'грунт';
...
room {
	...
	Careful {
		...
		description = "Электростанция состоит из солнечных батарей, управляющего блока и аккумуляторов.";
	};
	Careful {
...
room {
	...
	obj {
		...
		description = [[На постере изображена полуобнажённая девушка. Правой рукой она поправляет волосы. Левая рука небрежно лежит на гибкой талии. На голове у девушки надета корона, искрящаяся серебром. Белоснежная улыбка девушки излучает кокетство и власть.]];
		["before_Show,Give"] = function(s, w)
			if w ^ 'alex' then
				p [[Александр только покачал головой.]]
				return
			end
			return false
		end;
...
room {
	...
	Furniture {
		...
		dsc = false;
		description = [[Глядя на кровать ты понимаешь как ты устал.]];
