...
room {
	...
	["before_Walk,Enter"] = function(s, w)
		if w ^ '@e_to' then
			...
			p [[Ты едешь нв луноходе к лунному модулю Арго.]]
			move('buggy', 'moon1')
			return
		end
		return false
...
room {
	...
	dsc = function(s)
		...
		Здесь находится командный пульт. Рядом стоит кресло оператора. У противоположной стены расположен небольшой диванчик и стол.^^Ты можешь вернуться в научный модуль на востоке или пройти дальше в жилой модуль на север.]];
	end;
}:with {
	obj {
		-"пульт,провод*,дыр*";
