			...
			description = function(s)
				...
				if s:has 'on' then
					if not here()^'moon1' then
						...
						if nav_dragon then
							p [[Лунный модуль "Дракон" находится на юге.]]
						end
					else
		...
		daemon = function(s)
			...
			if gravity and _'moonmod'.height == 0 then
				...
				elseif here() == s:inroom() and not me():inside'buggy' and s:inside'buggy' then
					...
					move(s, here())
				elseif player_moved() and _'alex'.state == 7 and here():hasnt'cutscene' then
	...
	before_Talk = function(s)
		...
		if _'alex'.state == 7 then
			...
			if visited 'sklad' then
				...
				pn [[.. -- Ястреб, Заря... Мы ввели в пеленгатор лунохода частоту посадочной площадки "Дракона", обследуйте её тоже.]];
...
room {
	...
	dsc = function(s)
		...
		p [[Здесь установлен маяк. На севере находится электростанция.]];
	end;
	["before_Walk,Enter"] = function(s, w)
		if not nav_dragon and w ^ '@s_to' then
			return false
		end
		if w ^ '@e_to' or w ^ '@s_to' then
			if not me():inside'buggy' then
				p [[Слишком далеко, чтобы идти пешком.]]
				return
			end
			if w ^ '@e_to' then
				...
				move('buggy', 'moon1')
			elseif w ^ '@s_to' then
				p [[Ты едешь на луноходе к лунному модулю "Дракон".]]
				move('buggy', 'moon2')
			end
			return
		end
...
-- 1) ничего не находят
-- 2) осматривают разобранный Дракон
