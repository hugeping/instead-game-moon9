...
room {
	...
	before_Smell = [[Ты чувствешь душистый запах цветов.]];
	...
	obj {
		...
		before_Talk = function(s)
			...
			-- К сожалению, сны заканчиваются -- в голосы Ларисы чувствуется грусть.]];
		...
		description = function(s)
			...
			Лариса одета в белую рубашку и юбку, украшенную узором в виде маков. Она так молода!
...
cutscene {
	...
	text = {
		...
		... -- Где я? -- ты открываешь глаза и осматриваешься. Ты лежишь в кровати. Над тобой склонился Александр.^
		-- Ну наконец-то! -- радостно восклицает он. -- Мы всё ещё на "Луне-9". И похоже, это надолго.]];
		[[-- Что... Что произошло? Мне снился такой сон...^
		-- Ничего не помнишь? Нас открыл Ян. Иначе мы бы ещё час провозились бы с шлюзом. -- только тут ты замечаешь двух китайских космонавтов, сидящих на соседних кроватях.^
		-- И потом мы все шли пешком к пику, назад привезли тебя на луноходе. Ты был прямо как Ли, когда мы его с тобой нашли.]];
		[[-- А где... Где {$fmt em|они}? Ты понимаешь, о чём я говорю?^
		-- Мы не знаем. Та конструкция, которую мы... Которую {$fmt em|они} построили, полностью выгорела.
		Пока ты лежал в этой проклятой штуке что-то пошло не так. Ли и Ян говорят, что наваждение прошло. Я тоже пока не чувствую воздействия. Ушли ли они насовсем? Не знаю. Но если {$fmt em|они} вернутся, мы будем бороться.]];
		[[-- Нам нужно восстановить связь!^
		-- Полегче, командир. Отдыхай. Я знаю что ты сделал с нашим лунным модулем. Мы что-нибудь придумаем. Продержимся.^
		-- Нам не поверят, отстранят от полётов. Понимаешь это?]];
		[[-- Давай сначала вернёмся домой, командир.]];
	};
	next_to = 'theend';
}
gameover {
		...
		if _'alex'.state == 7 then
			...
			if visited 'sklad' then
				if visited'device' then
					p [[-- Заря, я Ястреб. Мы нашли космонавта при странных обстоятельсвах. Эвакуируем на базу.
					Подробности позже.]]
