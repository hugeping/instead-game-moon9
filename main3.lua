...
Ephe {
	...
	before_Talk = function(s)
		...
		if _'alex'.state == 5 then
			...
			pn "... Ястреб, Заря. Вы должны уже видеть это. Мы наблюдаем розовые вспышки закрывающие пик, диаметром до 7 километров."
			pn "-- Заря, я Ястреб. Вы понимаете что это такое?"
			pn "-- ... Ястреб, Заря. Не вполне. Возможно, это электростатические разряды в пыли. В любом случае, ЦУП принял решение не рисковать. Отведите модуль восточнее."
...
cutscene {
	...
	text = {
		...
		[[-- ... Ястреб, Заря, берите западнее, там чисто. Вы сами увидите это, ориентируйтесь визуально.^
