...
game.dsc = [[{$fmt b|ЛУНА-9}^^Интерактивная новелла для выполнения на средствах вычислительной техники.^Игра разработана с помощью ОС Plan9.^^Для помощи, наберите {$fmt em|помощь} и нажмите "ввод".]];
...
room {
	...
	obj {
		...
		before_Talk = function(s)
			...
			[[-- Ты будешь ждать меня? Я вернусь!^
...
room {
	...
	Careful {
		...
		before_Attack = function(s, w)
			...
			_'запчасти'.got = true
			take 'запчасти'
			p [[Содрогаясь от того, что ты делаешь, ты разбираешь панель и демонтируешь из неё нужные запчасти. Ты не знаешь точно, что именно ты делаешь. Но {$fmt em|они} подсказыват тебе.]]
		end;
		prog = 1;
		daemon = function(s)
			local m = _'moonmod'
		...
		if _'alex':visible() then
			...
			if here() ^ 'tech' then
				if not know_panel then
					p [[-- Что ты думаешь об этом?^
					-- Командир, смотри что с панелью управления!]];
				else
					p [[-- Что ты думаешь об этом?^
			...
			daemon = function(s)
				...
				if s:inside'трон' and s.step == 1 then
					...
					elseif s.step == 3 then
						...
						-- Я вижу, что ты готов, я прочитала ответ в твоём сердце. Ты -- мой. Когда вернёшься в свой мир, ты будешь знать всё, что нужно сделать. И ты сделаешь всё как надо! И тогда, вернувшись ко мне, ты получишь всё то, о чём мечтал, но не смеял надеяться.]]
						s.step = 4
