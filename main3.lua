...
room {
	...
	before_Wait = function(s)
		if dark_side() and _'comp'.speed > 2.0 then
			p(string.format("Скорость Арго-3 составляет %.02f км/с. Если не снизить скорость и не перейти на лунную орбиту, корабль после выхода из тени снова направится к Земле. Не время ждать -- время действовать!"))
			return
		end
	...
	Careful {
		...
		daemon = function(s)
			if s:once 'ack' then
				...
				p [[Ты видишь, что Александр и Сергей проснулись и потягиваются на своих креслах, разминя мышцы.]]
			...
			if s.ack then
				if not dark_side() and time() % 3 == 1 then
	...
	obj {
		...
		Careful {
			...
			before_Push = function(s)
				...
				if prog == 1 then
					if not dark_side() then
						p [[Нужно подождать, пока корабль не зайдёт в тень. Иначе, нарушится термальный контроль, который поддерживается вращением.]]
					else
						_'comp'.prog = false
				...
				elseif prog == 3 then
					_'comp'.prog = false
					_'модуль'.marsh = true
					p [[Корабль вздрогнул. Со стороны служебного отсека послышался сильный и низкий гул. Это запустился маршевый двигатель. 1, 2, 3, 4, 5... секунд. Вдруг, гул прекратился так же внезапно, как и начался. Что-то пошло не так! Двигатель должен был проработать 37 секунд!]]
				else
